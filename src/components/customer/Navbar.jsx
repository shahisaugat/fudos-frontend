...
const Navbar = () => {
    ...
    return (
        <nav className={NAV_CLASS}>
            ...
            <button onClick={toggleMobileMenu} className="md:hidden text-primary-foreground">
            ...
            {isMobileMenuOpen && (
                <div className="md:hidden absolute top-16 left-0 w-full bg-white shadow-lg z-50">
                    <div className="flex flex-col items-start  py-4 px-10">
                        ...
                        <Link to="/cart" className={ICON_CLASS + " mt-4"} onClick={toggleMobileMenu}>
                            <img src={basketIcon} alt="Basket" className="h-6 px-4" />
                        </Link>
                        {isLoggedIn ? (
                            <div className="flex flex-col items-center mt-4 ">
                                <img src={profileIcon} alt="Profile" className="h-8 mb-2" />
                                <button className="border-2 p-2 rounded-lg bg-[#EB5757] text-black" onClick={handleSignOut}>Sign Out</button>
                            </div>
                        ) : (
                            <Link to="/signin" className={ICON_CLASS + " mt-4"} onClick={toggleMobileMenu}>
                                <button className='border-2 p-2 rounded-lg bg-[#EB5757] text-black'>Login</button>
                            </Link>
                        )}
                    </div>
